...
public class QobuzAudioSourceManager extends MirroringAudioSourceManager implements HttpConfigurable {
	...
	public QobuzAudioSourceManager(String[] providers, AudioPlayerManager audioPlayerManager) {
		this(providers, unused -> audioPlayerManager, null, null, null);
	}

	public QobuzAudioSourceManager(String[] providers, Function<Void, AudioPlayerManager> audioPlayerManager) {
		this(providers, audioPlayerManager, null, null, null);
	}
	...
	public JsonBrowser getJson(String uri) throws IOException {
		...
		if (this.tokenTracker.getAppId() != null && !this.tokenTracker.getAppId().isEmpty()) {
			request.setHeader("x-app-id", this.tokenTracker.getAppId());
		}
		if (this.tokenTracker.getUserOauthToken() != null && !this.tokenTracker.getUserOauthToken().isEmpty()) {
	...
	private AudioItem getRecommendations(String id) throws IOException {
		...
		var recommendations = PulseLinkTools.fetchResponseAsJson(this.httpInterfaceManager.getInterface(), request);
